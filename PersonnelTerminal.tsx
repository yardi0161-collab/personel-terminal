...
function AbsensiForm({
  ...
  const attemptSubmit = () => {
    if (!valid) {
      ...
      showToast("Lengkapi semua kolom dan 4 foto bukti sebelum mengirim.", "warn");
      return;
    }
    ...
    uploadEntryPhotos(id, "absensi", photosToUpload).then((photos) => {
      if (photos.length) onLog({ ...entry, photos });
    });
    showToast("Absensi telah terkirim, tunggu hingga di-ACC.", "ok");
    reset(); // bersihkan draft & URL foto supaya siap untuk isian berikutnya
    onSubmitted(); // balik ke menu Absensi (memicu loading pindah halaman)
  };
  ...
  return (
    <div className={`pt-stack ${shake ? "pt-shake" : ""}`}>
      ...
      <button
        type="button"
        className="pt-submit"
        onClick={attemptSubmit} // TODO: kirim juga ke backend di sini (mis. FormData + draft.photos[i].file)
      >
        Kirim Laporan
      </button>
...
function CutiScreen({
  ...
  const submit = () => {
    ...
    showToast("Pengajuan cuti telah terkirim, tunggu hingga di-ACC.", "ok");
    setState(emptyCuti());
    onSubmitted();
  };
  ...
  const invalid = () => {
    setShake(true);
    setTimeout(() => setShake(false), 320);
    showToast("Lengkapi semua kolom sebelum mengirim.", "warn");
  };
...
function EvidenceScreen({
  ...
  const submit = () => {
    ...
    onLog(entry);
    uploadEntryPhotos(id, "evidence", state.photos).then((photos) => {
      if (photos.length) onLog({ ...entry, photos });
    });
  ...
  const invalid = () => {
    setShake(true);
    setTimeout(() => setShake(false), 320);
    showToast("Lengkapi semua kolom dan minimal 2 foto sebelum mengirim.", "warn");
  };
...
function CellScreen({
  ...
  const submit = () => {
    ...
    onLog(entry);
    uploadEntryPhotos(id, "cell", state.photos).then((photos) => {
      if (photos.length) onLog({ ...entry, photos });
    });
  ...
  const invalid = () => {
    setShake(true);
    setTimeout(() => setShake(false), 320);
    showToast("Lengkapi semua kolom dan minimal 1 screenshot sebelum mengirim.", "warn");
  };
...
function TilangScreen({
  ...
  const submit = () => {
    ...
    onLog(entry);
    uploadEntryPhotos(id, "tilang", state.photos).then((photos) => {
      if (photos.length) onLog({ ...entry, photos });
    });
  ...
  const invalid = () => {
    setShake(true);
    setTimeout(() => setShake(false), 320);
    showToast("Lengkapi semua kolom dan minimal 1 foto sebelum mengirim.", "warn");
  };
...
function ImpoundScreen({
  ...
  const submit = () => {
    ...
    onLog(entry);
    uploadEntryPhotos(id, "impound", state.photos).then((photos) => {
      if (photos.length) onLog({ ...entry, photos });
    });
...
/** Izin Cuti masuk bagian Absensi; Evidence/Cell/Tilang/Impound masuk bagian Laporan. */
function isAbsensiKind(k: LogKind) {
  return k === "absensi" || k === "cuti";
}

/** Rincian satu laporan untuk Admin: tab "Format" (kolom rapi) dan "Foto" (galeri bukti). */
function ReportDetail({ entry }: { entry: LogEntry }) {
  const [tab, setTab] = useState<"format" | "foto">("format");
  const rows = parseReportRows(entry.report);
  const photos = entry.photos ?? [];
  return (
    <div className="pt-report-detail">
      <div className="pt-detail-tabs">
        <button type="button" className={tab === "format" ? "is-active" : ""} onClick={() => setTab("format")}>
          Format
        </button>
        <button type="button" className={tab === "foto" ? "is-active" : ""} onClick={() => setTab("foto")}>
          Foto ({photos.length})
        </button>
      </div>
      {tab === "format" ? (
        <div className="pt-format-list">
          {rows.map((r, i) => (
            <div className="pt-format-row" key={i}>
              <b>{r.label}</b>
              <span>{r.value || "-"}</span>
            </div>
          ))}
        ...
        <div className="pt-photo-grid">
          {photos.map((url, i) => (
            <a key={url + i} href={url} target="_blank" rel="noreferrer" className="pt-photo-thumb">
              <img src={url} alt={`Foto ${i + 1}`} loading="lazy" referrerPolicy="no-referrer" />
            </a>
          ))}
        </div>
      ) : (
        <p className="pt-photo-hint">
          {entry.status === "pending" ? "Foto sedang diunggah / belum tersimpan." : "Tidak ada foto untuk laporan ini."}
        </p>
      )}
    </div>
  );
...
function ApprovalItem({
  ...
  const st = STATUS_META[entry.status];
  return (
    <li className={`pt-log-item ${open ? "is-open" : ""}`}>
      <button
        type="button"
        className="pt-log-head"
        aria-expanded={open}
...
/** Kotak per-pengirim di dalam antrean persetujuan. Selalu mulai tertutup. */
function ApprovalMemberBox({
  name,
  entries,
  logs,
  showTanggal,
  ...
  const avatarUrl = entries.find((l) => l.avatarUrl)?.avatarUrl ?? "";
  const pendingCount = entries.filter((l) => l.status === "pending").length;
  ...
  return (
    <section className={`pt-member-box ${open ? "" : "is-closed"}`}>
      ...
      >
        ...
        <span className="pt-seg-count">{pendingCount || entries.length}</span>
        ...
        <div className="pt-member-body">
          ...
          {showTanggal && tab === "tanggal" ? (
            ...
            <ul className="pt-log-list pt-member-list">
              {entries.map((entry) => (
                <ApprovalItem key={entry.id} entry={entry} onDecide={onDecide} />
              ))}
            </ul>
          )}
        </div>
...
function AdminApprovals({
  ...
  const absensiPendingCount = logs.filter((l) => isAbsensiKind(l.kind) && l.status === "pending").length;
  const laporanPendingCount = logs.filter((l) => !isAbsensiKind(l.kind) && l.status === "pending").length;

  const tabPool = logs.filter((l) => (tab === "absensi" ? isAbsensiKind(l.kind) : !isAbsensiKind(l.kind)));
  const pool = (filter === "pending" ? tabPool.filter((l) => l.status === "pending") : tabPool)
    .slice()
    .sort((a, b) => b.savedAt - a.savedAt);

  const names = Array.from(new Set(pool.map((l) => l.name))).sort((a, b) => a.localeCompare(b));

  return (
    <div className="pt-stack">
      <div className="pt-seg" role="tablist" aria-label="Bagian persetujuan">
        <button
          type="button"
          role="tab"
          aria-selected={tab === "absensi"}
        ...
        >
          Absensi
          <span className="pt-seg-count">{absensiPendingCount}</span>
        </button>
        <button
          type="button"
          role="tab"
          aria-selected={tab === "laporan"}
          className={`pt-seg-btn ${tab === "laporan" ? "is-active" : ""}`}
          onClick={() => setTab("laporan")}
        >
          Laporan
          <span className="pt-seg-count">{laporanPendingCount}</span>
        </button>
      </div>

      <div className="pt-subseg" role="tablist" aria-label="Filter status">
        <button
          type="button"
          role="tab"
          aria-selected={filter === "pending"}
          className={`pt-subseg-btn ${filter === "pending" ? "is-active" : ""}`}
        ...
        <button
          ...
          className={`pt-subseg-btn ${filter === "all" ? "is-active" : ""}`}
      ...
      {names.length === 0 ? (
        <section className="pt-card pt-log-empty">
          <p className="pt-log-empty-title">
            {filter === "pending" ? "Tidak ada yang menunggu persetujuan" : "Belum ada data"} di bagian{" "}
            {tab === "absensi" ? "Absensi" : "Laporan"}
          </p>
          <p className="pt-extra-text">
            {tab === "absensi"
              ? "Izin Cuti yang dikirim petugas akan muncul di sini."
              : "Evidence, Cell, Tilang, dan Impound yang dikirim petugas akan muncul di sini."}
          </p>
        </section>
      ) : (
        names.map((name) => (
          <ApprovalMemberBox
            key={name}
            name={name}
            entries={pool.filter((l) => l.name === name)}
            logs={logs}
            showTanggal={tab === "absensi"}
            onDecide={onDecide}
          />
        ))
      )}
...
/**
 * Layar loading bertema terminal "DATA_TRANSFER.EXE": ikon skull + teks status
 * yang berganti-ganti. Dipakai saat aplikasi pertama dibuka (teks bergilir lewat
 * BOOT_TEXTS) maupun setiap kali pindah halaman / setelah kirim laporan (fast,
 * cukup satu baris teks seperti "Membuka Absensi...").
 */
function DataTransferSplash({
  phase,
  fast,
  label,
}: {
  ...
  const messages = fast ? [label] : BOOT_TEXTS;
  ...
  return (
    <div
      className={`dt-splash ${fast ? "is-fast" : ""} ${phase === "out" ? "is-out" : ""}`}
      role="status"
      aria-label="Memuat"
    >
...
export default function PersonnelTerminal() {
  ...
  if (authState === "loading" || (authState === "out" && booting)) {
    return (
      <div className="pt-root">
        <style>{css}</style>
        <DataTransferSplash phase={phase === "done" ? "out" : phase} fast={false} label="" />
      </div>
    );
  }
  ...
  return (
    <div className="pt-root">
      ...
      {phase !== "done" && (
        <DataTransferSplash key={run} phase={phase} fast={fast} label={label} />
      )}
        ...
        <div className="pt-shell">
          ...
          <main className="pt-main">
            <div key={screen} className={`pt-page pt-page-${nav.dir}`}>
              ...
              {screen === "log" && (
                <LogScreen
                  logs={logs.filter((l) => l.name === profile.name)}
                  onRemove={removeLog}
                />
              )}
